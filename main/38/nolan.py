...
def get_movies():
    ...
    return [movie.attrib["title"] for movie in get_tree().iter(tag="movie")]


def get_runtimes():
    return [int(movie.attrib["runtime"].split()[0]) for movie in get_tree().iter(tag="movie")]
...
#     import numpy as np
#     return get_movies()[np.argmax(np.array(get_runtimes()))]
...
def get_movie_longest_runtime():
    """Call get_tree again and return the movie title for the movie with the longest
       runtime in minutes, for latter consider adding a _get_runtime helper"""
    tree = get_tree()
    movies = [(movie.attrib["title"], _get_runtime(movie)) for movie in tree.iter(tag="movie")]
